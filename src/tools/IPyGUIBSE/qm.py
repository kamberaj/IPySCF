...
############################################ Inputs ######################################################################
   def getConfigFileName(self):
       ...
       fName = fd.askopenfilename(filetypes=[("CRD file", "*.crd"), \
                                             ("COO file", "*.coo"), \
                                             ("PDB file", "*.pdb"), \
                                             ("MOL file", "*.mol"), \
                                             ("XYZ file", "*.xyz")], initialdir=fDir)
       tokens = fName.split('.')
       self.inputFileFmt = tokens[1].lower()
       self.inputConfigFile = fName
       return None
...
############################################################################     
class qmDialog(object):
   """
      QM Geometry Optimization class
   """
   def __init__ (self, master):
      # showInfo needs to know the master 5
      self.master = master
      ...
      # Create a Label to display a header
      self.headLbl = Label(self.frame, text="QM Geometry Optimization Options", relief=RIDGE)
      self.headLbl.pack(side=TOP, fill=X)
      ...
      Button(spacerFrame, text="Molecule Orbitals", command=self.plot_mol_orb).place(x=260, y=600)

      # Create the Cancel Button
      Button(spacerFrame, text="Cancel", command=self.Cancel).place(x=382, y=600)
...
############################################################################     
   ...
   def upload(self):
       ...
       print( self.ioFrame.inputConfigFile, self.ioFrame.inputFileFmt )
       
       if self.ioFrame.inputFileFmt == 'xyz':
          self.molecule = Compound(xyz=self.ioFrame.inputConfigFile)
       elif self.ioFrame.inputFileFmt == 'mol':  
          self.molecule = Compound(mol=self.ioFrame.inputConfigFile)
       elif self.ioFrame.inputFileFmt == 'mol2':  
          self.molecule = Compound(mol2=self.ioFrame.inputConfigFile)
          
       print(self.molecule.natoms)
       print(self.molecule.atomtypes)
       print(self.molecule.coordinates)
